...
This guide is for people who want to use more than one Codex account in Grillme. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Grillme](./install.md).
...
In Grillme Settings, name it something obvious:
...
In Grillme Settings, add another Codex provider:
...
Grillme shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Grillme treats it
