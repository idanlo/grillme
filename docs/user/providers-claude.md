...
This guide is for people who want to use more than one Claude setup in Grillme. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Grillme](./install.md).
...
In Grillme Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Grillme uses Claude Code's normal config directory.

When you set this field, Grillme points Claude Code at that directory with the
...
`~/.claude_personal_home/.claude`, which is not where Grillme looks.

Then add another Claude provider in Grillme:
...
Grillme only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Grillme keeps separate config directories isolated
...
Add or edit a Claude provider in Grillme Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Grillme stores the value as a server secret and does not
...
Grillme does not need a special Claude Code Router provider. Treat the router as a Claude
