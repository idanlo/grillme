...
import { type CursorSettings, type ModelSelection } from "@grillme/contracts";
import { sanitizeBranchFragment, sanitizeFeatureBranchName } from "@grillme/shared/git";
import { extractJsonObject } from "@grillme/shared/schemaJson";

import { TextGenerationError } from "@grillme/contracts";
  ...
  const runCursorJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      ...
      const runtime = yield* makeCursorAcpRuntime({
        ...
        cwd,
        clientInfo: { name: "grillme-git-text", version: "0.0.0" },
