import { ClaudeSettings, ProviderInstanceId } from "@grillme/contracts";
...
const ClaudeTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "grillme-claude-text-generation-test-",
...
function withFakeClaudeEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "grillme-claude-text-" });
