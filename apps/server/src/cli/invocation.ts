 ...
 * Detection is best-effort; callers must fail closed to a plain `grillme` command.
 */
...
/**
 * The `grillme` package spec to suggest. The literal spec the user typed (e.g.
 * `grillme@nightly`) is resolved away before our process starts, so re-derive it
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "grillme@nightly" : "grillme";
}

/**
 * Render a `grillme <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx grillme start` suggests
 * `npx grillme start`, a global install suggests `grillme start`, and a nightly build
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `grillme ${input.subcommand}`;
  }
