...
} from "@grillme/shared/git";
import {
  getChangeRequestTerminologyForKind,
  type ChangeRequestTerminology,
} from "@grillme/shared/sourceControl";

import { GitManagerError, GitPullRequestMaterializationError } from "@grillme/contracts";
...
import type { GitManagerServiceError } from "@grillme/contracts";
import * as GitVcsDriver from "../vcs/GitVcsDriver.ts";
import * as SourceControlProviderRegistry from "../sourceControl/SourceControlProviderRegistry.ts";
import { detectPrTemplate } from "../sourceControl/PrTemplateDetection.ts";
import type { ChangeRequest } from "@grillme/contracts";
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `grillme/pr-${pullRequest.number}/${suffix}`;
}
...
export const make = Effect.gen(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(
      tempDir,
      `grillme-pr-body-${process.pid}-${yield* randomUUIDv4(cwd)}.md`,
    );
