...
import { sanitizeBranchFragment } from "@grillme/shared/git";
import { detectSourceControlProviderFromRemoteUrl } from "@grillme/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("GRILLME_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("GRILLME_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("GRILLME_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("GRILLME_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("@grillme/server/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `grillme/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set GRILLME_BITBUCKET_EMAIL and GRILLME_BITBUCKET_API_TOKEN, or GRILLME_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
